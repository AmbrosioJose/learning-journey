...
class IntervalTree {
    private val nil: IntervalNode = IntervalNode(Interval(0,0))
    var root: IntervalNode = nil
     ...
     *
     * @param value
     * @return [IntervalNode] that matches the value otherwise returns null
     */
    fun search(interval: Interval): IntervalNode?{
        var currentNode = root
        while(currentNode != nil){
            if(currentNode.interval.intersects(interval))
                return currentNode
            else if(currentNode.interval.start <= interval.start)
    ...
    fun searchAll(interval: Interval): Array<IntervalNode>{
        val intervals = mutableListOf<IntervalNode>()

        fun collect(node: IntervalNode){
            if(node.interval.intersects(interval)){
                intervals.add(node)
            }
            if(node.left != nil && interval.start <= node.left!!.subtreeMax)
                collect(node.left!!)


            if(node.right != nil && Interval(node.interval.start, node.right!!.subtreeMax).intersects(interval))
                collect(node.right!!)
        }
...
//        println("Inserting: ${insertingNode.value} ")
        ...
        if(parentNode == nil)
            root = insertingNode
        else if(insertingNode.interval.start < parentNode!!.interval.start)
     ...
     * @param node
     */
    private fun rbInsertFixUp(node: IntervalNode){
        var focusNode = node
        while(focusNode.parent!!.color == COLOR.RED){
            if(focusNode.parent!! == focusNode.parent!!.parent!!.left){
                val uncle: IntervalNode = focusNode.parent!!.parent!!.right!!
                ...
                val uncle: IntervalNode = focusNode.parent!!.parent!!.left!!
                if(uncle.color == COLOR.RED){ // case 1
                    uncle.color = COLOR.BLACK
                    focusNode.parent!!.color = COLOR.BLACK
                    focusNode.parent!!.parent!!.color = COLOR.RED
     ...
     * Max fix up
     *
     * Starting at [node] calls node.updateMax() up to the root node
     *
     * @param node
     */
    private fun maxFixUp(node: IntervalNode){
        var currentNode = node
        while(currentNode != nil){
            currentNode.updateMax()
     ...
     * @param rotatingNode
     */
    fun leftRotate(rotatingNode: IntervalNode){
        ...
        rotatingNode.updateMax()
        newSubRoot.updateMax()
    }
     ...
     * @param rotatingNode
     */
    fun rightRotate(rotatingNode: IntervalNode){
    ...
    fun delete(interval: Interval){
        val deletingNode = search(interval) ?: return
        delete(deletingNode)
    }
     ...
     * @param node
     */
    private fun deleteFixUp(node: IntervalNode){
//        println("deleteFix Up ${if(node == nil) "nil" else node.toBasicString()}")
     ...
     * @param b
     */
    private fun transplant(a: IntervalNode, b: IntervalNode){
        if(a.parent == nil){
            root = b
     ...
     */
    fun minimum(node: IntervalNode): IntervalNode{
        var currentNode = node
        while(node.left != nil)
     ...
     */
    fun maximum(node: IntervalNode): IntervalNode{
        var currentNode = node
     ...
     * @return
     */
    fun predecessor(node: IntervalNode): IntervalNode? {
        return node
    }
     ...
     * @return
     */
    fun successor(node: IntervalNode): IntervalNode? {
        return node
    }
     ...
     * @param b
     */
    fun rbTransplant(a: IntervalNode, b: IntervalNode){
     ...
     * @param node
     */
    fun inOrderPrint(node: IntervalNode?){
        if(node != nil){
            inOrderPrint(node!!.left)
            print(" ${node.interval}(${node.color} parent = ${node.parent!!.interval}})")
     ...
     * @param root
     */
    fun printTree(root: IntervalNode?) {
        val lines: MutableList<List<String?>> = ArrayList()
        var level: MutableList<IntervalNode?> = ArrayList<IntervalNode?>()
        var next: MutableList<IntervalNode?> = ArrayList<IntervalNode?>()
        level.add(root)
        var nn = 1
        var widest = 0
        while (nn != 0) {
            ...
            lines.add(line)
            val tmp: MutableList<IntervalNode?> = level
            level = next
            next = tmp
            next.clear()
        }
     ...
     * Print tree
     *
     * Prints the tree in a visual format starting at the root of this [IntervalTree]
     */
    fun printTree(){
        printTree(root)
    }
